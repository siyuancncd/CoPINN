...
def apply_model_copinn(epoch, num_epochs,apply_fn, params, *train_data):
    def residual_loss(epoch, num_epochs,params, t, x, y, z, source_term):
        def compute_loss(params, t, x, y, z, source_term, mean=True):
            ...
            if mean:
                ...
                return (utt - uxx - uyy - uzz + u**2 - source_term)**2

        # # self-paced learning 
        loss = compute_loss(params, t, x, y, z, source_term, mean=False)
        grad_fn = jax.grad(compute_loss, argnums=(1, 2, 3, 4))
        grad_t, grad_x, grad_y, grad_z = grad_fn(params, t, x, y, z,source_term)

        grad_t_broadcasted = grad_t[:, None, None, None]  # Extend to (Nc, 1, 1, 1)
        grad_x_broadcasted = grad_x[None, :, None, None]  # Extend to (1, Nc, 1, 1)
        grad_y_broadcasted = grad_y[None, None, :, None]  # Extend to (1, 1, Nc, 1)
        grad_z_broadcasted = grad_z[None, None, None, :]  # Extend to (1, 1, 1, Nc)

        flattened_loss = loss.flatten()

        V = get_SPL_V(grad_t_broadcasted, grad_x_broadcasted, grad_y_broadcasted, grad_z_broadcasted, epoch, num_epochs)

        new_loss = flattened_loss * V

        return jnp.mean(new_loss) 

    def get_SPL_V(grad_t, grad_x, grad_y, grad_z, epoch, num_epochs):
        ...
        difficult = jnp.sqrt(grad_t**2 + grad_x**2 + grad_y**2 + grad_z**2).squeeze()
    ...
    loss_fn = lambda params: residual_loss(epoch, num_epochs,params, tc, xc, yc, zc, uc) + \
...
if __name__ == '__main__':
    ...
    # input data settings
    parser.add_argument('--nc', type=int, default=16, help='the number of training points for each axis: 16 32 64 128 256')
    ...
    for e in trange(1, args.epochs + 1):
        ...
        if e % args.log_iter == 0:
            ...
            with open(os.path.join(result_dir, 'log (loss, error).csv'), 'a') as f:
                f.write(f'{loss}, {error}, {best_error}, {best_rmse}\n')
