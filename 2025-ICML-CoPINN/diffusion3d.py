...
def apply_model_copinn(epoch, num_epochs, apply_fn, params, *train_data):
    def residual_loss(epoch, num_epochs, params, t, x, y, alpha=0.05):
        def compute_loss(params, t, x, y, mean=True):
            # compute u
            ...
            if mean:
                ...
                return (ut - alpha * (ux**2 + u*uxx + uy**2 + u*uyy))**2

        # # self-paced learning 
        loss = compute_loss(params, t, x, y, mean=False)
        grad_fn = jax.grad(compute_loss, argnums=(1, 2, 3))
        grad_t, grad_x, grad_y = grad_fn(params, t, x, y)
    ...
    def get_SPL_V(grad_t, grad_x, grad_y, epoch, num_epochs):
        ...
        difficult = jnp.sqrt(grad_t**2 + grad_x**2 + grad_y**2).squeeze()
    ...
    loss_fn = lambda params: residual_loss(epoch, num_epochs, params, tc, xc, yc) + \
...
if __name__ == '__main__':
    ...
    # input data settings
    parser.add_argument('--nc', type=int, default=16, help='the number of training points for each axis: 16 32 64')
    ...
    parser.add_argument('--plot_iter', type=int, default=5000, help='plot result every...')
