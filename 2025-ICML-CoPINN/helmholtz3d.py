...
def apply_model_copinn(epoch, num_epochs, apply_fn, params, *train_data):
    def residual_loss(epoch, num_epochs, params, x, y, z, source_term, lda=1.):
        def compute_loss(params, x, y, z, source_term, lda=1., mean=True):
            # compute u
            ...
            loss = ((uzz + uyy + uxx + lda*u) - source_term)**2 
            if mean:
                return jnp.mean(loss)
            else:
                return loss
        
        loss = compute_loss(params, x, y, z, source_term, lda=1.,mean=False)
        grad_fn = jax.grad(compute_loss, argnums=(1, 2, 3))
        grad_x, grad_y, grad_z = grad_fn(params, x, y, z, lda, source_term)

        grad_x_broadcasted = grad_x[:, None, None]  # Extend to (Nc, 1, 1)
        grad_y_broadcasted = grad_y[None, :, None]  # Extend to (1, Nc, 1)
        grad_z_broadcasted = grad_z[None, None, :]  # Extend to (1, 1, Nc)

        flattened_loss = loss.flatten()

        V = get_SPL_V(grad_x_broadcasted, grad_y_broadcasted, grad_z_broadcasted, epoch, num_epochs)
...
if __name__ == '__main__':
    ...
    # input data settings
    parser.add_argument('--nc', type=int, default=64, help='the number of training points for each axis: 16 32 64 128 256')
    ...
    for e in trange(1, args.epochs + 1):
        ...
        args_dict = vars(args)
        num_epochs = jnp.array(args_dict['epochs'])
        epoch = jnp.array(e)
        loss, gradient = apply_model_copinn(epoch, num_epochs, apply_fn, params, *train_data)
